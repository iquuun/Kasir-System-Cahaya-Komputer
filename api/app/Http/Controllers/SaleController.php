...
class SaleController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Cek duplikat no_pesanan (jika diisi)
        if (!empty($validated['no_pesanan'])) {
            $existing = Sale::where('no_pesanan', $validated['no_pesanan'])->first();
            if ($existing) {
                return response()->json(['message' => 'No Pesanan "' . $validated['no_pesanan'] . '" sudah terdaftar di invoice ' . $existing->invoice . '.'], 422);
            }
        }
        ...
        try {
            ...
            $sale = Sale::create([
                ...
                'no_pesanan' => $validated['no_pesanan'] ?? null,
    ...
    public function update(Request $request, Sale $sale)
    {
        ...
        // Cek duplikat no_pesanan (jika diisi, kecualikan transaksi ini sendiri)
        if (!empty($validated['no_pesanan'])) {
            $existing = Sale::where('no_pesanan', $validated['no_pesanan'])->where('id', '!=', $sale->id)->first();
            if ($existing) {
                return response()->json(['message' => 'No Pesanan "' . $validated['no_pesanan'] . '" sudah terdaftar di invoice ' . $existing->invoice . '.'], 422);
            }
        }
