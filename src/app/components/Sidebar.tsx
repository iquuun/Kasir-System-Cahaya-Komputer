    ...
    >
      ...
      {hovered && (
        <div
          ...
          onMouseLeave={handleMouseLeave}
        >
          <div className="bg-gray-900 text-white rounded-lg shadow-2xl border border-white/10 overflow-hidden min-w-[160px]">
            {/* Header label */}
            ...
            {/* Arrow pointing left */}
            <div className="absolute left-0 top-3 -translate-x-full w-0 h-0 border-t-[5px] border-t-transparent border-b-[5px] border-b-transparent border-r-[6px] border-r-gray-900" />
          </div>
        </div>
      )}
    </div>
...
export default function Sidebar() {
    ...
    <>
            ...
            <div key={item.path} className="mb-0.5 w-full">
              {hasSubItems ? (
                <button
                  ...
                  className={`flex items-center justify-between px-3 py-2 rounded-md w-full transition-all text-xs ${
                    location.pathname.startsWith(item.path)
                      ...
                      : `text-sidebar-foreground/85 hover:bg-white/10 hover:text-white font-medium ${(!isCollapsed || isMobile) ? 'hover:translate-x-1' : ''}`
                ...
                >
                  ...
                  {(!isCollapsed || isMobile) && (
                    <div className="shrink-0 text-sidebar-foreground/50">
                      ...
                      <NavLink
                        ...
                        className={`flex items-center gap-2 py-1.5 px-3 rounded-md text-[11px] font-medium transition-all ${
                            isSubActive
                              ...
                              : 'text-sidebar-foreground/70 hover:text-white hover:bg-white/10 hover:translate-x-0.5'
                          }`
                        }
                      >
      ...
      <div className={`border-t border-white/10 shrink-0 ${(isCollapsed && !isMobile) ? 'p-1.5' : 'p-3'}`}>
          <button
            ...
            className={`flex items-center px-3 py-2 rounded-md w-full text-red-200/90 hover:bg-red-500/20 hover:text-red-100 transition-all text-xs font-semibold ${(!isCollapsed || isMobile) ? 'hover:translate-x-1' : ''} ${(isCollapsed && !isMobile) ? 'justify-center gap-0' : 'gap-2.5'}`}
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <div className={`hidden md:flex h-screen bg-gradient-to-b from-blue-500 via-blue-600 to-indigo-950 dark:from-[#0b1226] dark:via-[#0f172a] dark:to-[#020617] text-sidebar-foreground flex-col shrink-0 transition-all duration-300 ease-in-out border-r border-sidebar-border/30 print:hidden relative ${isCollapsed ? 'w-14' : 'w-44'}`}>
