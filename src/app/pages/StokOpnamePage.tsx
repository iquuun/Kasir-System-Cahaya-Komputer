      ...
      {activeTab === 'opname' ? (
        <div className="animate-in fade-in duration-300 space-y-6">
          ...
          <div className="grid grid-cols-3 md:grid-cols-3 gap-2 md:gap-3">
             <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-2 md:p-3">
               <p className="text-[8px] md:text-[10px] uppercase font-bold text-gray-500 tracking-wider mb-0.5 line-clamp-1">Total Terfilter</p>
               <p className="text-base md:text-xl font-bold text-[#3B82F6]">{filteredItems.length}</p>
             </div>
             <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-2 md:p-3">
               <p className="text-[8px] md:text-[10px] uppercase font-bold text-gray-500 tracking-wider mb-0.5 line-clamp-1">Item Selisih</p>
               <p className="text-base md:text-xl font-bold text-red-600">{itemsWithDiff.length}</p>
             </div>
             <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-2 md:p-3 bg-blue-50/50">
               <p className="text-[8px] md:text-[10px] uppercase font-bold text-gray-500 tracking-wider mb-0.5 line-clamp-1">Status Opname</p>
               <p className="text-[10px] md:text-base font-bold text-gray-800 tracking-tight line-clamp-1">READY TO SAVE</p>
             </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3 space-y-3">
            ...
            <div className="space-y-3 md:hidden max-h-[60vh] overflow-y-auto">
              {currentItems.map((item) => (
                <div key={item.product_id} className={`bg-white rounded-xl shadow-sm border p-3 space-y-3.5 ${item.selisih !== 0 ? 'border-orange-200 bg-orange-50/10' : 'border-gray-100'}`}>
                  <div className="flex justify-between items-start">
                    <h4 className="font-bold text-xs text-gray-800 line-clamp-2">{item.product_name}</h4>
                    <span className={`px-2 py-0.5 rounded text-[10px] font-black shrink-0 ${item.selisih > 0 ? 'bg-green-100 text-green-700' : item.selisih < 0 ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-400'}`}>
                      {item.selisih > 0 ? '+' : ''}{item.selisih}
                    </span>
                  </div>
                  ...
                  <div className="border-t border-gray-50 pt-2 flex justify-between items-center text-xs">
                    <div className="flex flex-col">
                      <span className="text-[10px] text-gray-400 font-bold uppercase">Stok Sistem</span>
                      <span className="font-semibold text-gray-700">{item.stok_sistem}</span>
                    </div>
                    <div className="flex items-center gap-1">
                      <span className="text-[10px] text-gray-400 font-bold uppercase mr-1">Stok Fisik:</span>
                      <input 
                        type="number" 
                        value={item.stok_fisik === '' ? '' : item.stok_fisik} 
                        onChange={(e) => updateStokFisik(item.product_id, e.target.value)} 
                        className="w-16 text-center border border-gray-200 rounded text-xs font-bold py-1 bg-white" 
                      />
                    </div>
                  </div>
                </div>
        ...
        <div className="animate-in fade-in duration-300 space-y-4">
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
             ...
             <div className="hidden md:block overflow-x-auto rounded-xl border border-gray-50">
             ...
             <div className="space-y-3 md:hidden">
               {historyLoading ? (
                 <div className="p-8 text-center text-xs text-gray-400 bg-white rounded-xl border border-gray-100">Memuat riwayat...</div>
               ) : movements.length === 0 ? (
                 <div className="p-12 text-center text-xs text-gray-400 italic bg-white rounded-xl border border-gray-100">Tidak ada data pergerakan</div>
               ) : (
                 movements.map(m => (
                   <div key={m.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 space-y-2">
                     <div className="flex justify-between items-start">
                       <div>
                         <h4 className="font-bold text-xs text-gray-800">{m.product?.name || 'Produk Dihapus'}</h4>
                         <p className="text-[9px] text-gray-400 uppercase tracking-tight mt-0.5">KODE: {m.product?.kode || '-'}</p>
                       </div>
                     ...
                     <div className="border-t border-gray-50 pt-2.5 flex justify-between items-center text-[11px] text-gray-600">
                       <div className="flex flex-col">
                         <span className="text-[9px] text-gray-400 font-bold uppercase tracking-wider mb-0.5">Waktu</span>
                       ...
                       <div className="flex flex-col items-end">
                         <span className="text-[9px] text-gray-400 font-bold uppercase tracking-wider mb-0.5">Sumber</span>
                         <span className="text-[9px] font-black border border-gray-200 bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded uppercase">{m.sumber}</span>
                       </div>
                     </div>
                     
                     {m.keterangan && (
                       <div className="bg-gray-50 p-2 rounded-lg mt-1 text-[10px] text-gray-500 italic">
                         {m.keterangan}
                       </div>
                     )}
                   </div>
