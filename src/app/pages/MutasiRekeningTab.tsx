            ...
            {currentFlows.map((flow) => (
              <div key={flow.id} className="p-2.5 space-y-1.5 hover:bg-muted transition-colors">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className={`inline-flex items-center gap-0.5 px-2 py-0.5 rounded-full text-[9px] font-black tracking-wider ${flow.tipe === 'masuk' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                    {flow.tipe === 'masuk' ? <TrendingUp size={9} /> : <TrendingDown size={9} />}
                    {flow.tipe === 'masuk' ? 'MASUK' : 'KELUAR'}
                  </span>
                </div>
                ...
                <div className="flex justify-between items-center text-[11px]">
                  <div className="font-bold text-foreground capitalize">
                    ...
                    {flow.sumber === 'biaya_operasional' && (
                      <>
                        {flow.keterangan && flow.keterangan.toLowerCase().includes('gaji') ? (
                          <span className="ml-1.5 inline-flex border border-purple-200 text-purple-600 px-1 py-0.2 text-[8px] rounded uppercase bg-purple-50">GAJI</span>
                        ) : (
                          <span className="ml-1.5 inline-flex border border-amber-200 text-amber-600 px-1 py-0.2 text-[8px] rounded uppercase bg-amber-50">OPS</span>
                        )}
                      </>
                    )}
                    {flow.sumber === 'gaji_karyawan' && (
                  ...
                  <span className={`font-black text-xs ${flow.tipe === 'masuk' ? 'text-emerald-600' : 'text-rose-600'}`}>
                    {flow.tipe === 'masuk' ? '+' : '-'} Rp {Math.abs(Number(flow.nominal)).toLocaleString('id-ID')}
                  </span>
                </div>
