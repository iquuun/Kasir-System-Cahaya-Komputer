...
import { Monitor, ShoppingBag, Settings, Plus, Trash2, Camera, HelpCircle, Save, Percent, Receipt, ChevronUp, ChevronDown } from 'lucide-react';
import api from '../api';
...
export default function KalkulatorPage() {
    ...
    const tabFromUrl = searchParams.get('tab') as 'rakitan' | 'satuan' | 'pengaturan' | null;

    const [activeTab, setActiveTab] = useState<'rakitan' | 'satuan' | 'pengaturan'>(tabFromUrl || 'rakitan');
    const [isConfigExpanded, setIsConfigExpanded] = useState(() => window.innerWidth > 768);
    ...
    const handleTabChange = (tab: 'rakitan' | 'satuan' | 'pengaturan') => {
        setActiveTab(tab);
    ...
    const renderConfigForm = () => {
        const storeName = storeConfigs.find(s => s.id === selectedStoreId)?.name || 'Belum Pilih Toko';
        ...
        return (
            <div className="bg-card border rounded-xl shadow-sm overflow-hidden mb-4">
                ...
                >
                    ...
                    <div className="flex items-center gap-1.5 md:hidden text-blue-600 font-bold text-xs">
                        <span>{isConfigExpanded ? 'Sembunyikan' : 'Atur Biaya'}</span>
                        {isConfigExpanded ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                    </div>
                </div>

                {/* Form fields (collapsible container) */}
                                        ...
                                        <div key={item.id} className="bg-muted border border-border p-1.5 rounded-lg relative group" style={{ zIndex: rakitanItems.length - idx }}>
                                            ...
                                            <div className="flex md:hidden flex-col gap-2 w-full p-0.5">
                                                ...
                                                <div className="flex items-center gap-2 pl-7">
                                                    ...
                                                    <div className="flex-1 flex items-center gap-1.5">
                                                        ...
                                                        <div className="relative flex-1">
                                                            ...
                                                            <input 
                                                                type="text" 
                                                                min="0" 
                                                                value={item.modal === 0 ? '' : item.modal.toLocaleString('id-ID')} 
                                                                onChange={(e) => { 
                                                                    const rawValue = e.target.value.replace(/\./g, '');
                                                                    const numValue = parseInt(rawValue, 10);
                                                                    const n = [...rakitanItems]; n[idx].modal = isNaN(numValue) ? 0 : Math.max(0, numValue); setRakitanItems(n); 
                                                    ...
                                                    <button 
                                                        onClick={() => setRakitanItems(rakitanItems.filter(r => r.id !== item.id))} 
                                                        className="p-1.5 bg-red-50 text-red-500 hover:bg-red-100 border border-red-100 rounded-lg flex-shrink-0"
                                                    >
                                                        <Trash2 size={14} />
                                                    </button>
                                                </div>
