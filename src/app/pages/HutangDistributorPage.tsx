    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-2 md:p-3">
          <p className="text-[8px] md:text-[10px] uppercase font-bold text-gray-500 tracking-wider mb-0.5 line-clamp-1">Total Hutang Keseluruhan</p>
          <p className="text-base md:text-xl font-bold text-[#3B82F6]">Rp {totalHutang.toLocaleString('id-ID')}</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-2 md:p-3">
          <p className="text-[8px] md:text-[10px] uppercase font-bold text-gray-500 tracking-wider mb-0.5 line-clamp-1">Total Terbayar</p>
          <p className="text-base md:text-xl font-bold text-green-600">Rp {totalTerbayar.toLocaleString('id-ID')}</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-2 md:p-3">
          <p className="text-[8px] md:text-[10px] uppercase font-bold text-gray-500 tracking-wider mb-0.5 line-clamp-1">Total Sisa Hutang</p>
          <p className="text-base md:text-xl font-bold text-red-600">Rp {totalSisa.toLocaleString('id-ID')}</p>
        </div>
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-2 md:p-3 flex flex-col justify-center">
          <p className="text-[8px] md:text-[10px] uppercase font-bold text-gray-500 tracking-wider mb-0.5 line-clamp-1">Jumlah Invoice Hutang</p>
          <p className="text-base md:text-xl font-bold text-gray-800 leading-tight">{hutangList.length}</p>
          <p className="text-[8px] md:text-[9px] text-gray-400 font-medium">Belum lunas sepenuhnya</p>
        </div>
      </div>

      {/* Hutang Per Distributor */}
      <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-2">
        <h3 className="text-[11px] uppercase font-bold text-gray-800 mb-2">Hutang Aktif Per Distributor</h3>
          ...
          {Object.entries(hutangPerDistributor)
            ...
            .map(([distributor, sisa]) => (
              <div key={distributor} className="px-2 py-1.5 bg-red-50/50 border border-red-100 rounded">
                <p className="font-bold text-[10px] text-gray-800 truncate" title={distributor}>{distributor}</p>
                <p className="text-[11px] font-bold text-red-600">Rp {sisa.toLocaleString('id-ID')}</p>
              </div>
            ))}
            {Object.keys(hutangPerDistributor).length === 0 && (
                <p className="text-[10px] text-gray-500 italic py-1">Tidak ada hutang aktif.</p>
            )}
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-4 items-center">
        <div className="flex-1 relative w-full">
          ...
          <input 
            ...
            className="w-full pl-10 pr-4 py-2 bg-white border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all shadow-sm"
            value={searchQuery}
        ...
        <div className="relative z-20 w-full sm:w-auto">
          <button 
            ...
            className="flex items-center gap-2 px-3 py-2 bg-white border border-gray-200 rounded-lg text-[11px] font-bold text-gray-700 hover:bg-gray-50 focus:ring-2 focus:ring-blue-500 w-full sm:w-[265px] justify-between whitespace-nowrap shadow-sm"
          >
      ...
      <div className="hidden md:block bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-3 py-2.5 border-b border-gray-100 flex items-center justify-between">
          <h3 className="text-[11px] uppercase font-bold text-gray-800">Detail Pembelian / Hutang</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50/50 border-b border-gray-100">
              <tr>
                <th className="text-left px-2 py-2 text-[10px] uppercase tracking-wider font-bold text-gray-500">Invoice</th>
                <th className="text-left px-2 py-2 text-[10px] uppercase tracking-wider font-bold text-gray-500">
                  <select
                    value={filterDistributor}
                    onChange={(e) => setFilterDistributor(e.target.value)}
                    className="bg-transparent border-none outline-none font-bold text-gray-500 p-0 text-[10px] uppercase tracking-wider cursor-pointer hover:text-gray-700"
                  >
                ...
                <th 
                  className="text-left px-2 py-2 text-[10px] uppercase tracking-wider font-bold text-gray-500 cursor-pointer hover:bg-gray-200 transition-colors"
                ...
                <th className="text-right px-2 py-2 text-[10px] uppercase tracking-wider font-bold text-gray-500">Total</th>
                <th className="text-right px-2 py-2 text-[10px] uppercase tracking-wider font-bold text-gray-500">Terbayar</th>
                <th className="text-right px-2 py-2 text-[10px] uppercase tracking-wider font-bold text-gray-500">Sisa</th>
                <th className="text-center px-2 py-2 text-[10px] uppercase tracking-wider font-bold text-gray-500">
                  <select
                    value={filterStatus}
                    onChange={(e) => setFilterStatus(e.target.value as any)}
                    className="bg-transparent border-none outline-none font-bold text-gray-500 p-0 text-[10px] uppercase tracking-wider cursor-pointer hover:text-gray-700 text-center"
                  >
                ...
                <th className="text-center px-2 py-2 text-[10px] uppercase tracking-wider font-bold text-gray-500">Aksi</th>
              </tr>
            </thead>
                ...
                filteredPurchases.map((purchase) => {
                  ...
                  return (
                    <tr key={purchase.id} className="hover:bg-blue-50/30 transition-colors">
                      ...
                      <td className="px-2 py-2 text-[11px] text-gray-700 truncate max-w-[120px]">{purchase.distributor?.name || '-'}</td>
                      <td className="px-2 py-2 text-[11px] text-gray-600">
                        {new Date(purchase.tanggal).toLocaleDateString('id-ID')}
                      </td>
                      <td className="px-2 py-2 text-[11px] text-right font-bold text-gray-800">
                      ...
                      <td className="px-2 py-2 text-center">
                        {isLunas ? (
                            ...
                            <div className="w-12 bg-gray-100 rounded-full h-1 mt-0.5">
                                <div
      ...
      <div className="space-y-3 md:hidden">
        {filteredPurchases.length === 0 ? (
          ...
          filteredPurchases.map((purchase) => {
            ...
            return (
              <div key={purchase.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 space-y-3">
                <div className="flex justify-between items-start">
                  <div>
                    <h4 className="font-bold text-sm text-gray-800">{purchase.invoice || '-'}</h4>
                    <p className="text-xs text-gray-500">{purchase.distributor?.name || '-'}</p>
                  </div>
                  {isLunas ? (
                ...
                <div className="border-t border-gray-50 pt-2 space-y-1.5 text-xs">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-gray-800 font-bold">Rp {total.toLocaleString('id-ID')}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className={sisa > 0 ? 'text-red-600 font-bold' : 'text-gray-400'}>
            ...
            <form onSubmit={handlePay} className="p-4 space-y-4 flex-1 overflow-y-auto">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-2.5 py-1.5 border border-gray-200 rounded-lg focus:ring-1 text-xs font-medium bg-gray-50 focus:ring-blue-500 outline-none"
                />
              </div>
              <div className="flex justify-end gap-3 pt-4 sticky bottom-0 bg-white mt-auto md:mt-0 border-t border-gray-100 md:border-none md:pt-4">
                <button
                  ...
                  className="hidden md:block px-3 py-2 text-gray-600 hover:bg-gray-100 rounded-lg text-xs font-bold"
                >
                  Batal
                </button>
                <button
