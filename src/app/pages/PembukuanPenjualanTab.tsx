...
export default function PembukuanPenjualanTab() {
  ...
  if (loading) return (
    <div className="space-y-6 animate-pulse">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-2 md:gap-3">{[...Array(5)].map((_, i) => (<div key={i} className="bg-white rounded-xl shadow-sm border border-gray-100 p-3"><div className="h-3 bg-gray-200 rounded w-20 mb-2" /><div className="h-6 bg-gray-200 rounded w-28" /></div>))}</div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          <table className="w-full whitespace-nowrap">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th 
                  className="text-left px-3 py-2 text-[10px] uppercase font-bold text-gray-500 tracking-wider cursor-pointer hover:bg-gray-200 transition-colors"
        ...
        <div className="block md:hidden divide-y divide-gray-100 bg-white">
          {currentItems.length === 0 ? (
            ...
            return (
              <div key={sale.id} className={`p-2.5 space-y-2 hover:bg-gray-50 transition-colors ${isChecked ? 'bg-blue-50/40' : ''}`}>
                {/* Row 1: Index, Invoice, Date, and Checkbox */}
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    <span className="text-gray-400 text-[10px] font-bold">#{index + 1 + (currentPage - 1) * itemsPerPage}</span>
                ...
                <div className="text-[11px] space-y-1">
                  <div>
                    ...
                    {isEditing ? (
                      ...
                      <span className="font-semibold text-gray-800">{sale.nama_barang_manual || '-'} <span className="text-[10px] text-gray-500 font-normal">(Qty: {totalQty})</span></span>
                    )}
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-2 gap-x-3 gap-y-1 bg-gray-50 p-2 rounded-lg text-[10px] text-gray-700">
                  <div className="flex justify-between">
                    <span className="text-gray-400">Harga Jual:</span>
                    <span className="font-semibold">{hrgJual.toLocaleString('id-ID')}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-400">Harga Modal:</span>
                    {isEditing ? (
                      <input
                        type="text"
                        value={formatNumber(editForm.harga_modal_manual)}
                  ...
                  <div className="flex justify-between">
                    <span className="text-gray-400">Laba Bersih:</span>
                    <span className={`font-bold ${labaBersih > 0 ? 'text-green-600' : 'text-gray-400'}`}>
                      {labaBersih !== 0 ? labaBersih.toLocaleString('id-ID') : '-'}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-400">ADM Market:</span>
                    <span className="font-semibold text-orange-600">
                      {amdMarket > 0 && masukDP > 0 ? amdMarket.toLocaleString('id-ID') : '-'}
                    </span>
                  </div>
                  <div className="flex justify-between bg-blue-50/50 px-1 py-0.5 rounded">
                    <span className="text-gray-400">Masuk/DP:</span>
                    {isEditing ? (
                      <input
                        type="text"
                        value={formatNumber(editForm.masuk_dp)}
                  ...
                  <div className="flex justify-between bg-purple-50/50 px-1 py-0.5 rounded">
                    <span className="text-gray-400">Keluar/TF:</span>
                    {isEditing ? (
                      <input
                        type="text"
                        value={formatNumber(editForm.keluar_tf)}
                  ...
                  <div className="flex justify-between col-span-2 border-t border-gray-200/60 pt-1 mt-0.5">
                    <span className="text-gray-400 font-bold">Sisa Hutang Market:</span>
                ...
                <div className="flex justify-between items-center pt-1.5 border-t border-gray-100">
                  ...
                  {isEditing ? (
                    <div className="flex gap-1.5">
                      ...
                      <button onClick={cancelEdit} className="px-2 py-1 bg-gray-100 hover:bg-gray-200 text-gray-600 rounded text-[10px] font-bold">
                        Batal
                      </button>
                    </div>
                  ) : (
